###############################################################################
# evaluate_fit_v5_step6.R
# Evaluate TRACE-Q step6 (unaudited) across 20 runs for K=4 and K=6
...
for (r in 1:N_RUNS) {
  ...
  fpath_K6 <- file.path(run_dir, "step6_Q_matrix_K6.csv")
  if (file.exists(fpath_K6)) {
    Q6 <- read.csv(fpath_K6, row.names = 1)
    complete_K6 <- c(complete_K6, is_complete(Q6))
    results_K6 <- rbind(results_K6, extract_fit(data, Q6))
  }
  
  fpath_K4 <- file.path(run_dir, "step6_Q_matrix_K4.csv")
  if (file.exists(fpath_K4)) {
    Q4 <- read.csv(fpath_K4, row.names = 1)
...
summary_df <- data.frame(
  Q_Matrix = c(
    sprintf("TRACE-Q K=4 step6 (n=%d)", nrow(results_K4)),
    "TSQE K=4",
    sprintf("TRACE-Q K=6 step6 (n=%d)", nrow(results_K6)),
    "TSQE K=6",
    "Expert K=8"
  ),
