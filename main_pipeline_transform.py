...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run the end-to-end scaffold pipeline: structure discovery -> cluster transformation -> "
            "transformation-aware taxonomist -> optional taggers."
        )
    )
    parser.add_argument("--dossiers", default="outputs/step2_verifier_verified_item_dossiers.jsonl", help="Verified dossiers JSONL")
    parser.add_argument("--prompts_dir", default="prompts/v2", help="Prompt directory for taxonomist stages")
    ...
    parser.add_argument("--tagger_prompts_dir", default="prompts/v1", help="Prompt directory for taggers")
    ...
    taxonomist_cmd = [
        py,
        "main_taxonomist_transform.py",
        "--input", args.dossiers,
        "--scaffold_brief", "outputs/step2_5_scaffold_brief.json",
        "--transformations", "outputs/step2_6_cluster_transformations.json",
        "--out", "outputs/step3_skill_codebook_transform.json",
        "--out_experts", "outputs/step3_expert_codebooks_transform.json",
        "--out_align", "outputs/step3_supervisor_align_output_transform.json",
        "--out_supervisor", "outputs/step3_supervisor_output_transform.json",
    ...
    if args.run_taggers:
        tagger_cmd = [
            ...
            "--codebook", "outputs/step3_skill_codebook_transform.json",
            "--out_dir", "outputs/step4_tagger_votes_transform",
    ...
    print("\nPipeline complete.")
    print("Final codebook: outputs/step3_skill_codebook_transform.json")
    if args.run_taggers:
        print("Tagger votes: outputs/step4_tagger_votes_transform/")
