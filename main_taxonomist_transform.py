"""
Transformation-aware Taxonomist Stage.

Pipeline:
1. Read scaffold brief
2. Read cluster transformation review outputs
3. Inject both scaffold and transformation summary into Expert and Supervisor prompts
4. Produce final codebook via existing Expert Committee + Supervisor workflow
"""
...
def run_expert(
    ...
    prompt_text = _augment_prompt(raw_prompt, scaffold_brief, transform_outputs)
    expert = Expert(
        llm=llm,
        expert_id=expert_id,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a skill codebook using scaffold + cluster transformation review."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        ...
        help="Cluster transformation outputs JSON",
    )
    parser.add_argument(
        "--out",
        default="outputs/step3_skill_codebook_transform.json",
        help="Output final codebook JSON",
    )
    parser.add_argument(
        "--out_experts",
        default="outputs/step3_expert_codebooks_transform.json",
        help="Output individual expert codebooks",
    )
    parser.add_argument(
        "--out_align",
        default="outputs/step3_supervisor_align_output_transform.json",
        help="Output supervisor alignment result",
    )
    parser.add_argument(
        "--out_supervisor",
        default="outputs/step3_supervisor_output_transform.json",
    ...
    transform_raw = read_json(args.transformations) if Path(args.transformations).exists() else {}
    ...
    print("=== Phase 1: Transformation-aware Expert Committee ===")
    ...
    llm = LLMClient()
    align_prompt_path = str(Path(args.prompts_dir) / "supervisor_align.txt")
    aligner = SupervisorAlign(llm=llm, prompt_path=align_prompt_path)
    ...
    consolidate_prompt_text = _augment_prompt(raw_consolidate, scaffold_brief, transform_outputs)
    ...
    print(f"Transformation-aware final codebook saved to: {args.out}")
